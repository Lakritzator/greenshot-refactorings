...
{
    /// <summary>
    /// Renderer for ImageShape (including BackgroundShape)
    /// </summary>
    public class ImageRenderer : IShapeRenderer
    {
        public bool CanRender(IShape shape)
        {
            return shape is ImageShape;
        }
        ...
        public void Render(Graphics graphics, IShape shape)
        {
            ...
            var imageData = imageShape.ImageData;
            if (imageData == null)
            {
                return;
            }

            // For now, only support BitmapImageData
